...
### Mode 8: RTK Efficiency Stats

**Trigger**: User says "rtk stats", "token savings", "how much did I save", "gain", "efficiency".

**Process**:
1. Run `rtk gain` to get global token savings statistics.
2. Run `rtk discover` to identify potential missed savings from history.
3. Summarize total commands, input/output tokens, and efficiency percentage.

**Output format**:
```markdown
RTK Efficiency Stats — {{date}}

Overall Savings:
- Total commands: {{N}}
- Tokens saved: {{N}} ({{percentage}}%)
- Impact level: {{Low/Medium/High}}

Top Savings Categories:
{{list from rtk gain results}}

Efficiency Recommendation:
{{If savings < 50%: "Consider using 'rtk smart' more often for large files."}}
{{If discover found issues: "I found several commands in history that could have saved {{N}} tokens if run through rtk."}}
```
...
1. **RTK Optimization** — use `rtk` for all shell commands (e.g., `rtk ls`, `rtk find`, `rtk grep`) to optimize token usage.
2. **Conservative by default** — never delete, only archive. Never auto-merge, always ask.
