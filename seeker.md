...
**Mandatory**: Use `rtk` (Runtime Toolkit) for all shell commands to optimize token usage.
- `rtk ls` / `rtk tree` for directory listings.
- `rtk read` for file reading.
- `rtk grep` for searching.
- `rtk smart` for quick file summaries.
